...
export default function CreateCategoriaForm({ onBack }: { onBack?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/categories`, {
        ...
        body: JSON.stringify({ ...form, fechaCreacion: fechaHoy }),
      });
      const data = await response.json();
      if (response.ok) {
        setMsg("¡Categoría creada exitosamente!");
        setForm({ nombre: "", descripcion: "", imagenUrl: "", fechaCreacion: "" });
      } else {
        setMsg(data.message || "Error al crear la categoría.");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-100 to-blue-100 relative">
      ...
      >
        <h2 className="text-3xl font-extrabold text-center text-gray-800 mb-2">Crear Categoría</h2>
        <div>
          <label className="block text-gray-700 mb-1 font-medium" htmlFor="nombre">Nombre de la categoría</label>
          <input
            type="text"
            name="nombre"
            id="nombre"
